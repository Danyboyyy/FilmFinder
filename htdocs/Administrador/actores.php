<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && ISSET($_REQUEST["action"]) && $_REQUEST["action"]=="nuevo") {
      if ($_POST['nombre']!='') {        
        $sql = "SELECT MAX(ID_Actor) as ID_Actor FROM Actor";
        $result = mysqli_query($conn, $sql);
        if($row = mysqli_fetch_assoc($result))
          $maxID = $row["ID_Actor"] + 1;
        
        $Nombre = mysqli_real_escape_string($conn, $_POST['nombre']);
        $sql = "INSERT INTO Actor (ID_Actor, Nombre)
        VALUES(".$maxID.",'$Nombre')";

        if (mysqli_query($conn, $sql)) {
            echo "<p style=\"color:green\">Se ha agregado al actor</p>";
        } else {
    ...
    if (ISSET($_REQUEST["action"]) && $_REQUEST["action"]=="borrar") {
      ...
      if ($_REQUEST['ID_Actor']>0) {
        $sql = "DELETE FROM Actor WHERE ID_Actor=".$_REQUEST['ID_Actor'];
        $result = mysqli_query($conn, $sql);
...
<html>
  ...
  <body>
    <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
      Nombre del actor: <input type="text" name="nombre" value="<?php echo $Nombre;?>"><br><br>
    ...
    <?php
      ...
      if (mysqli_num_rows($result) > 0) {
          ...
          while($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<td><a href=\"".$_SERVER['PHP_SELF']."?action=borrar&ID_Actor=".$row["ID_Actor"]."\">Delete</a></td>";
            echo "</tr>";
          }
      } 
      echo "</table>";
